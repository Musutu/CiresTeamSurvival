"""Render one native view per town district (world-scale: 14 districts), the full-realm overview and the escort, and verify the fixture checks."""
...
def main() -> int:
    ...
    with (folder / "console.log").open("wb") as output:
        ...
        try:
            ...
            try:
                ...
                child.kill()
                code = child.wait(timeout=5)
            failure = "Environment gallery exceeded its 720-second process bound"
    contents = log.read_text(encoding="utf-8", errors="replace") if log.exists() else ""
    match = re.search(r"CIRE_ENVIRONMENT_GALLERY_PASS captures=(\d+) checks=(\d+) directory=(.+)", contents)
    ...
    passed = bool(passed and match and len(captures) == int(match.group(1)) >= 7)
    report = dict(passed=passed, exitCode=code, seconds=round(time.monotonic()-started, 2),
                  checks=int(match.group(2)) if match else None, failure=failure, errors=errors, log=str(log), captures=captures)
