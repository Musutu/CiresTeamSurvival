...
BODIES = {  # ChampionAttacks02 body folder -> set (its drafted profile's preset)
    ...
    "PolyPaladin": "sword_shield",  # paladin-hq: the Polyphoria plate body (Iron Warden, both Relic Paladins)
}
# paladin-hq: Fab champion bodies that are not ChampionAttacks02 bodies (no Tripo action clips): mesh -> body folder.
# RetargetFabAnimations.py retargets onto them like the Tripo bodies and writes FabAnimations.json "bodies" so
# CireFabAnimation::FolderFor finds them. The mesh is the leader of ChampionArtBindings.fab.json "humanoid" rows.
FAB_BODIES = {
    "/Game/Polyphoria/Polyphoria/CharacterParts_HeavyArmour/Meshes/UE5/Male/UpperBody/SK_ma_pala_combine_a.SK_ma_pala_combine_a": "PolyPaladin",
}
...
def main():
    ...
    out = {
        "_comment": "Generated by Tools/BuildFabAnimMap.py. clips.<set>_<key>: source clip; timing measured by RetargetFabAnimations.py. "
                    "locomotion.<set>: idle + walk/run in 8 directions. bodies: ChampionAttacks02 folder -> set. fabBodies: Fab body mesh -> folder. styles: WeaponLoadouts preset -> set.",
        "clips": clips, "locomotion": {k: v for k, v in loco.items() if v}, "bodies": BODIES, "fabBodies": FAB_BODIES, "extraSets": EXTRA_SETS,
