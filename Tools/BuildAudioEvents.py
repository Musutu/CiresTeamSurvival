...
def generated_cues() -> "OrderedDict[str, dict]":
    ...
    out["death.creature"] = cue(["Auras/AUR_Snarl"], .6, [.7, .8], cooldown=.08, attenuation="combat", maxVoices=3, priority=14, bus="voice", combat=True,
                                with_=["death.creature.body"])
    # fab-coverage: pack-only body-fall layers under the (vocal, CC0) creature and boss deaths; silent without the packs.
    out["death.creature.body"] = cue([], .7, None, cooldown=.08, attenuation="combat", maxVoices=3, priority=13, **combat)
    out["death.boss.body"] = cue([], .95, None, cooldown=2.0, attenuation="large", maxVoices=2, priority=79, **combat)
    ...
    out["death.boss"] = cue(["SFX/SFX_PackLeaderRoar"], 1.0, [.72, .78], cooldown=2.0, attenuation="large", priority=80, bus="voice", combat=True,
                            duck=.55, duckSeconds=2.5, with_=["death.boss.body"])
    ...
    # fab-coverage: the loot chest lands (drop + bounce) and opens (ACireLootDrop, client side, visible chests only).
    out["loot_chest_land"] = cue(["Footsteps/FS_Thump_{01..05}"], .6, [.85, .95], cooldown=.1, attenuation="combat", maxVoices=3, priority=20, bus="sfx")
    out["loot_chest_open"] = cue(["Ambience/AMB_Door_{01..04}"], .5, [1.15, 1.25], cooldown=.1, attenuation="close", maxVoices=3, priority=25, bus="sfx")
    # fab-coverage: custom banners (ECireBanner::Custom) used the synthesized horn; now a cue like every other banner.
    out["banner_custom"] = cue(["Ambience/AMB_Bell_{01..03}"], .5, [1.0, 1.04], cooldown=2.0, bus="ui")
...
HUD_LEGACY = OrderedDict([("0", "level_up"), ("1", "aggro_taken"), ("2", "ui_threat"), ("3", "ui_aggro_lost"), ("4", "ui_target"),
                          ("5", "banner_custom"), ("6", "banner_cleared")])
...
def main() -> int:
    ...
    new_root["hudLegacy"] = OrderedDict([("notes", "ACireHUD::PlayWowSound index -> cue. Every index routes to a cue; the synthesized tone is only the fallback when cue data is missing.")] + list(HUD_LEGACY.items()))
