...
def run(engine, waves, duration, timeout, enemies=5):
    ...
    for wave in waves:
        ...
        forecast=simulate(tuning,wave=wave,level=wave,enemy_count=enemies,seconds=duration)
...
def main():
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('--engine',type=Path,default=Path('F:/UE_5.8'));p.add_argument('--waves',default='1,10,30');p.add_argument('--seconds',type=float,default=90);p.add_argument('--timeout',type=float,default=240);p.add_argument('--enemies',type=int,default=5)
    a=p.parse_args()
    try:
        ...
        print('MEASURED_REPORT '+str(run(a.engine,waves,a.seconds,a.timeout,a.enemies)))
