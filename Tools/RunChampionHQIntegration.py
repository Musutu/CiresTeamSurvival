"""Run Tools/IntegrateChampionHQ.py with temporary skeleton core redirects (the Bridge animation-redirector workaround).
...
def redirects():
    lines = []
    for folder in sorted((ROOT / "Content" / "TripoModels").glob("CTS_ChampHQ_*")):
...
def main():
    backup = INI.with_suffix(".ini.champion-hq.bak")
    ...
    try:
        ...
        run_editor([ENGINE + "UnrealEditor.exe", str(ROOT / "CiresTeamSurvival.uproject"), "-unattended", "-nosplash", "-Multiprocess",
                    ...
                    "-ExecCmds=py " + (ROOT / "Tools" / "IntegrateChampionHQ.py").as_posix(),
                    "-abslog=" + str(ROOT / "Saved" / "Logs" / "ChampionHQ-Integrate.log")], 5400).check_returncode()
    finally:
        shutil.copy2(backup, INI)
        backup.unlink()
    report = json.loads((ROOT / "Saved" / "ChampionHQIntegration.json").read_text(encoding="utf-8"))
