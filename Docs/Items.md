...
Status: `feat/items-v2`, 25 September 2026 (items v2 on top of the progression-shop prototype). **Every
number here is a proposal for Eric to review.** The catalog is data (`Content/Data/Items.json`); the rules
...
1. **Universal primary scaling.** Every ability, summon and construct scales off the champion's primary
   ...
   **flat** health, mana, armor and ward, regeneration, attack speed, cooldown reduction and move speed.
   No item grants attack damage, spell power or an off-stat (STR/AGI/INT directly). Only completed
   (legendary) items may add **mitigation specials** (`damageReduction` %, `damageBlock` per hit),
   critical chance or lifesteal. `Cires::Items::ValidateStatPolicy` enforces this: Items.json fails to
   load otherwise.
...
| attackSpeed, cooldownReduction | also apply to the owner's summons and constructs (the scaling agent implements that side) | `BasicAttack()` / `Recalculate` |
| moveSpeed | % move speed (boots; client-predicted from the replicated bag and buffs) | `ACireHero::Tick` |
| healthRegen / manaRegen / energyRegen | flat per second | `CireItems::ApplyRegen` |
...
| critChance, lifesteal *(legendary only)* | special bonuses of completed items | `Recalculate` / `CireCombat::ApplyDamage` |
| strength, agility, intelligence, attackDamage, spellPower | **rejected** on items (the engine keys still exist for skills and buffs) | `ValidateStatPolicy` |
...
- **Galeborn Twinstep** (560g): +12% move speed, +10% attack speed; **Double Dash**: +1 dodge-roll charge.
...
- Mana items that matter: Pilgrim's Idol (+3/s), Aether Conduit (+4/s), Moonwell Codex (+500 mana,
  +5/s, 20% refund), Chalice/Censer/Reliquary regen, Elixir of Black Sorcery.
...
| Elixir of Wrath (`elixir_of_wrath`) | 300g (300g) | - | - | 3 min: +6 primary stat, +12% attack speed. | **Use:** For 3 minutes: +6 primary stat and +12% attack speed. Persists through death. |
| Elixir of Black Sorcery (`elixir_of_black_sorcery`) | 300g (300g) | - | - | 3 min: +6 primary stat, +3 mana/s. | **Use:** For 3 minutes: +6 primary stat and +3 mana per second. Persists through death. |
...
| Bone Dagger (`bone_dagger`) | 155g (155g) | - | +12 AS % | Attack speed (you and your summons). | - |
...
| Sandglass Charm (`sandglass_charm`) | 190g (190g) | - | +6 CDR % | Cooldown reduction (you, your summons and constructs). | - |
| Pilgrim's Idol (`pilgrims_idol`) | 130g (130g) | - | +3 Mana/s | Mana regeneration. | - |

### Epic components (7)

| Item (`id`) | Total (recipe) | Build path | Stats | One-line effect | Passive / active |
|---|---|---|---|---|---|
| Serrated Cleaver (`serrated_cleaver`) | 465g (140g) | Rusted Longsword + Bone Dagger | +8 Primary, +15 AS % | Primary stat and attack speed. | - |
...
| Aether Conduit (`aether_conduit`) | 420g (130g) | Ashwood Wand + Pilgrim's Idol | +6 Primary, +4 Mana/s | Primary stat and mana regeneration. | - |
| Oathkeeper's Charm (`oathkeeper_charm`) | 450g (120g) | Sandglass Charm + Bloodstone Shard | +8 CDR %, +250 HP | Cooldown reduction and health. | - |

### Path-defining uniques (one per champion) (6)

| Item (`id`) | Total (recipe) | Build path | Stats | One-line effect | Passive / active |
|---|---|---|---|---|---|
| Artificer's Heartforge (`artificers_heartforge`) *(path unique)* | 960g (380g) | Aether Conduit + Ashwood Wand | +16 Primary, +10 CDR %, +200 Mana | Construct path: +1 construct per recipe; constructs are shielded. | **Second Forge:** +1 live construct of each recipe (turrets, pylons, traps, skitters). **Aegis Plating:** Your constructs have +20% health and spawn with a shield worth 30% of their maximum health. |
| Soulbinder's Crook (`soulbinders_crook`) *(path unique)* | 935g (360g) | Grimoire of Whispers + Bone Dagger | +18 Primary, +300 Mana, +10 AS % | Summon path: summons and pets +35% health and damage. | **Shepherd of Souls:** Your summons and pets have +35% health and deal +35% damage. |
...
| Shackles of the Pale King (`shackles_of_the_pale_king`) *(path unique)* | 950g (370g) | Mantle of Ash + Sandglass Charm | +8 Primary, +350 HP, +25 Ward, +10 CDR % | Control path: your crowd control lasts 30% longer. | **Pale Dominion:** Your stuns, slows and silences last 30% longer; you deal 12% more damage to enemies under your control. |
| Sigil of Apotheosis (`sigil_of_apotheosis`) *(path unique)* | 1000g (390g) | Oathkeeper's Charm + Gauntlet of the Ox | +10 Primary, +350 HP, +15 CDR % | Ultimate path: your ultimate gains an extra effect. | **Apotheosis:** Your ultimate gains an extra effect (its upgrade is listed on the ultimate's tooltip). |
| Stormhowl Ravager (`stormhowl_ravager`) *(path unique)* | 1005g (380g) | Serrated Cleaver + Gauntlet of the Ox | +18 Primary, +20 AS %, +200 HP | Attack path: basic attacks hit harder and cleave 40%. | **Howling Cleave:** Basic attacks deal +15% damage and cleave 40% of it to enemies within 2.5 m of the target. |
...
| Stalker's Treads (`stalkers_treads`) *(boots)* | 515g (180g) | Road-Worn Boots + Bone Dagger | +12 MS %, +18 AS % | Boots: speed and attack speed. | - |
| Veilwalker Slippers (`veilwalker_slippers`) *(boots)* | 515g (145g) | Road-Worn Boots + Sandglass Charm | +12 MS %, +10 CDR % | Boots: speed and cooldown reduction. | - |
...
| Galeborn Twinstep (`twinstep_treads`) *(boots)* | 560g (380g) | Road-Worn Boots | +12 MS %, +10 AS % | Unique boots: +1 dodge-roll charge (double dash). | **Double Dash:** +1 dodge-roll charge: roll twice back to back; charges refill one at a time. |
...
| Banner of the Vigil (`banner_of_the_vigil`) *(unique)* | 800g (220g) | Oathkeeper's Charm + Boiled Leather Jerkin | +300 HP, +20 Armor, +10 CDR % | Active: allies within 9 m gain +250 armor for 10 s. | **Active - Raise the Vigil:** You and allies within 9 m gain +250 armor for 10 seconds. 90 s cooldown. |
| Chalice of Mercy (`chalice_of_mercy`) *(unique)* | 770g (230g) | Aether Conduit + Nightglass Shard | +6 Primary, +300 Mana, +4 Mana/s | Active: heal a small area for 200. | **Active - Outpouring:** Heal yourself and allies within 4.5 m for 200. 30 s cooldown. |
| Lifebinder's Reliquary (`lifebinders_reliquary`) *(unique)* | 820g (260g) | Aether Conduit + Bloodstone Shard | +8 Primary, +300 HP, +3 Mana/s | Active: heal one ally for 1,000. | **Active - Bind the Living:** Heal your targeted ally (or yourself) within 12 m for 1,000. 90 s cooldown. |

### Other legendaries (12)

| Item (`id`) | Total (recipe) | Build path | Stats | One-line effect | Passive / active |
|---|---|---|---|---|---|
| Nightfall Reaver (`nightfall_reaver`) *(unique)* | 965g (330g) | Serrated Cleaver + Rusted Longsword | +16 Primary, +20 AS %, +20 Crit % | +20% crit chance; crits deal 175%. | **Nightfall:** Critical strikes deal 175% damage instead of 150%. |
| Sanguine Sabre (`sanguine_sabre`) *(unique)* | 950g (320g) | Bloodletter + Rusted Longsword | +16 Primary, +250 HP, +12 Lifesteal % | 12% attack lifesteal; abilities heal 6% of their damage. | **Blood Tithe:** Your abilities also heal you for 6% of the damage they deal. |
| Headsman's Greataxe (`headsmans_greataxe`) *(unique)* | 945g (330g) | Bloodletter + Bone Dagger | +18 Primary, +12 AS %, +200 HP | +20% damage to targets under 30% health. | **Execution:** Deal 20% more damage to targets below 30% health. |
| Wraithstring (`wraithstring`) *(unique)* | 920g (300g) | Serrated Cleaver + Bone Dagger | +10 Primary, +32 AS % | Every third attack deals 35 bonus damage. | **Wraith Echo:** Every third basic attack deals 35 bonus damage. |
...
| Moonwell Codex (`moonwell_codex`) *(unique)* | 850g (300g) | Grimoire of Whispers + Pilgrim's Idol | +10 Primary, +500 Mana, +5 Mana/s | Abilities refund 20% of their mana cost. | **Moonwell:** Your abilities refund 20% of their mana cost. |
| Hourglass of Ages (`hourglass_of_ages`) *(unique)* | 890g (280g) | Oathkeeper's Charm + Ashwood Wand | +8 Primary, +12 CDR %, +200 HP | Active: 60% damage reduction and a 10% heal. | **Active - Borrowed Time:** For 4 seconds take 60% less damage, and heal 10% of your maximum health. 60 s cooldown. |
...
| Stoneheart (`stoneheart`) *(unique)* | 740g (300g) | Bloodstone Shard + Bloodstone Shard + Gauntlet of the Ox | +600 HP, +6 Primary, +4 HP/s, +6 DR % | 6% less damage from everything; a last-stand shield below 30%. | **Unbroken:** Dropping below 30% health grants 40% damage reduction for 4 seconds. 45 s cooldown. |
...
| Censer of Dawn (`censer_of_dawn`) *(unique)* | 650g (230g) | Aether Conduit | +10 Primary, +5 Mana/s | Your healing is 15% stronger. | **Consecrated Mending:** Your healing is 15% stronger. |
| Ravenfeather Mantle (`ravenfeather_mantle`) *(unique)* | 505g (220g) | Hexweave Cloak + Bone Dagger | +20 Ward, +10 AS %, +5 MS % | Active: burst of speed that shakes off slows. | **Active - Scatter:** Gain 40% move speed for 3 seconds and shake off slows. 40 s cooldown. |
...
4. **Crit and lifesteal** kept only as completed-item specials (Nightfall Reaver, Sanguine Sabre);
   say the word and they become passives or go away.
