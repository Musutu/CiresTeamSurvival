...
| stoneborn | RuneSentinelHQ one_hand, GraniteCrusherHQ unarmed, BastionGolem sword_shield, DeepforgeRunesmith spell, StonebornForgelord two_hand, StonebornColossus unarmed, EtherMote unarmed | 7/7 assigned retargeted, 15 roles each. Segmentation candidate DeepforgeRunesmith (arm-bound low 10%: the fused hammer head on ring_03_r) checked close up (face_deepforge_runesmith): hammer sits in both hands, no visible stretch, not segmented. Stone/chain beards are rigid by design (no sway). |
| feral_kin | WerebearMaulerHQ unarmed, TuskedBehemothHQ sword_shield, FeralShaman spell | 3/3 assigned retargeted, 15 roles each (FeralUrsoth, FeralMammoth have no set assigned and keep their clips). Sway: Tusked Behemoth's hanging trunk (mask Saved/MonsterGallery/20260926-095751, tusks stay rigid). Segmentation candidate TuskedBehemothHQ (leg-bound high 4%) checked close up: shield, belts and trunk read cleanly, no visible stretch, not segmented. FeralMammoth (far 7%) has no set and no visible artifact in the race lineup. |
| npc_bodies | IronboundBruiserV2 two_hand, IronboundBruiser two_hand, HollowShieldbearerV2 sword_shield, HollowShieldbearer sword_shield, BlightCaster spell, BlightCasterB spell, BarbedHunter bow, BarbedHunterB crossbow, HollowSiegebreaker two_hand, GravemawPackLeader two_hand | 10/10 NPCMeshes bodies retargeted (BarbedHunterB crossbow 11 roles by design, the rest 15). Segmentation candidate GravemawPackLeader (far 12.4% on spine_02) checked close up (face_gravemaw_pack_leader): it is the wolf-pelt cape, which stretches into a long band on the two-hand swing but reads as a flowing pelt, not torn geometry; logged, not segmented (Tripo re-segment would be the fix if Eric wants it). HollowShieldbearer thumb 'far' flag = the shield in the hand, fine. |
