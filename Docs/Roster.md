...
## Ability icons (shared lookup)

Every skill has a painted 256 px icon: all 37 draft-pool skills (each a unique
glyph + school palette) and all 63 planned roster skills. They are original
procedural art (no licensed sources) made by `Tools/BuildAbilityIcons.py`
(Pillow) and imported by `python Tools/RunAbilityIcons.py [--pylib DIR]` as
`/Game/UI/Abilities/T_<skill id>`; the tool also writes
`Content/Data/AbilityIcons.json` (school, glyph, accent colour per id).

Use it everywhere an ability is shown (draft screen, skill-offer cards, action
bars, tooltips):

```cpp
#include "CireAbilityIcons.h"
UTexture2D* Icon = CireAbilityIcons::Texture(Id);      // == CireUIStyle::FindAbilityIcon(Id)
FLinearColor Tint = CireAbilityIcons::Accent(Id);      // school colour (card trims, sigil fallback)
FString School = CireAbilityIcons::School(Id);         // "fire", "holy", "frost"...
CireAbilityIcons::Draw(Painter, Id, X, Y, Size);       // painted icon, or tinted sigil fallback
// Action bars: FCireIconSlot Slot; Slot.IconTexture = CireAbilityIcons::Texture(Id); Slot.Tint = CireAbilityIcons::Accent(Id);
```

Add a new skill to `POOL` (or it inherits a keyword/delivery glyph if planned)
in `Tools/BuildAbilityIcons.py`, then rerun the tool; the script refuses to
build if a pool skill has no design or two pool skills share one.
...
- **Portraits** are bust renders of the real champion meshes (body, weapons,
  ...
  The tool measures highlight clipping per champion and trims exposure (pale
  granite / felfire / spirit bodies); the trims are stored in
  `Content/UI/Draft/Portraits/Exposure.json` and also applied to the live
  preview. Tiles show the full champion name (two lines when needed).
...
- **Style.** Columns, preview, details, team cards and the LOCK IN button use
  the shared style kit (`CireUIStyle`: frames, buttons, icon slots) and the
  kit list uses the shared painted ability icons with passive/ultimate frames.
