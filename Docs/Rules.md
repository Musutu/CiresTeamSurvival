...
- Each STR gives 10 maximum health, 0.1 armor and 0.1 spell ward (Eric's ruling, 25 September 2026; it was 25 health and no defense). Each champion also has a flat base health of 15 x its authored starting STR (`Cires::StartingBaseHealth`), so level-1 health is unchanged (500 for a 20-STR champion, 250 for a 10-STR one) and only the per-point growth is lower. Each INT gives 30 maximum mana; each AGI adds one percentage point to the attack-speed multiplier. The primary stat adds one basic-attack damage per point, plus the weapon baseline. The playable champions use a 12-damage baseline weapon; the reusable module's default tuning is 10.
...
- The starter catalog contains 22 active skills, four passives, and eleven ultimate options. Learned skills cannot reappear. Active/passive/ultimate capacities are enforced independently, and invalid or undersized catalogs fail explicitly rather than violating slot restrictions.
- Every skill carries Tank / DPS / Support role tags (a bit mask, so cross-class skills carry several). A champion's progression records a primary role plus optional hybrid roles; offers and `LearnSkill` only accept skills whose tags intersect them. See [RoleDrafts.md](RoleDrafts.md).
- **Opening skill point:** a new champion's first offer is due at level 1: four primary-role actives (no passives or ultimates). Later offers arrive at levels 3, 6 ... 21 (`BreakpointForSkill`). Class baseline traits (Support Mending Strikes / -20% damage / +10% attack speed, Tank flat -10 per hit after armor, DPS 10% base crit) are defined in `Cires::Traits`. See [RoleDrafts.md](RoleDrafts.md).
