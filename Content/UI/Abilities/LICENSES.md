...
Ability, passive, ultimate, trait, status-effect (`T_status_<name>`) and role (`T_role_<name>`)
...
Every other `T_<id>` texture here is original procedural art painted by `Tools/BuildAbilityIcons.py`
(no licensed sources).
