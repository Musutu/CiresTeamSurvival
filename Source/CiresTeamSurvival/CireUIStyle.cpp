...
{
    // champion-draft fix: T_Glow is white RGB with an alpha falloff; additive canvas blending ignores alpha
    // (solid squares), so blend translucently.
    if(const FAssets& A=Assets();A.Glow)P.Tex(A.Glow,X-W*.25f,Y-H*.25f,W*1.5f,H*1.5f,Color,0,0,1,1,false);
...
void CireUIStyle::IconSlot(const FCireUIPainter& P,float X,float Y,float S,const FCireIconSlot& Slot,double Time)
{
    ...
    if(Slot.bGlow)
    {
        ...
        for(int32 I=0;I<4;++I)
        {
            ...
            if(A.Glow)P.Tex(A.Glow,Pt.X-S*.2f,Pt.Y-S*.2f+Dy,S*.4f,S*.4f,FLinearColor(1.f,.8f,.3f,.9f),0,0,1,1,false); // champion-draft: alpha falloff needs translucent
        }
