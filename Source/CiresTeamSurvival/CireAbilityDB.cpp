    ...
    TMap<FName,TArray<FCireModifier>>& OutModifiers,FString& Error)
{
    ...
    for(const auto& Pair:(*Abilities)->Values)
    {
        ...
        D.Types=Strings(J,TEXT("types"));D.Section=Str(J,TEXT("section"));D.EffectTags=Strings(J,TEXT("effectTags"));D.Champions=Strings(J,TEXT("champions"));D.SignatureOf=Strings(J,TEXT("signatureOf"));
